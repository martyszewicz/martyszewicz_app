...
def login_user(request):
    if request.method == "GET":
        ...
        if user is not None:
            login(request, user)
            messages.info(request, f"Witaj {username}!")
...
def logout_user(request):
    if request.user.is_authenticated:
        logout(request)
        messages.info(request, "Wylogowałeś się poprawnie")
        return redirect('index')
    else:
        return redirect('index')
...
def new_user(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            messages.info(request, f"Użytkownik {form.cleaned_data['username']} został zarejestrowany")
        else:
            for field, errors in form.errors.items():
                for error in errors:
                    messages.info(request, f'Błąd: {error}')
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if new_email != user.email:
                ...
                messages.success(request, f"Adres email użytkownika {user.username} został zaktualizowany.")
            if new_password:
                user.set_password(new_password)
                update_session_auth_hash(request, user)
                user.save(update_fields=['password'])
                messages.success(request, f"Hasło użytkownika {user.username} zostało zaktualizowane.")
            return redirect('users')
        else:
            for field, errors in form.errors.items():
                for error in errors:
...
def user_delete(request, user_name):
    ...
    messages.info(request, f"Użytkownik '{user_name}' został pomyślnie usunięty")
    return redirect('users')
...
def search_movies(request):
    if request.method == "POST":
        ...
        if not title:
            ...
            if not title:
                messages.info(request, 'Proszę podać tytuł')
                return redirect("index")
        try:
            response = requests.get(f'https://search.imdbot.workers.dev/?q={title}')
            if response.status_code != requests.codes.ok:
                messages.info(request, f"Nie można połączyć z bazą danych, kod błędu: {response.status_code}")
            ...
            return JsonResponse({'error': f'Wystąpił błąd: {e}'})
...
def save_movie(request, movie_id):
    ...
    if record:
        messages.info(request, "Ten film już jest zapisany w bibliotece 'Ulubione'")
    else:
        FavouritesMovies.objects.create(user=request.user, movie_id=movie_id)
        messages.info(request, "Film zapisano pomyślnie w bibliotece 'Ulubione'")
...
def movie_delete(request, movie_id):
    ...
    messages.info(request, f"Film został pomyślnie usunięty z ulubionych")
